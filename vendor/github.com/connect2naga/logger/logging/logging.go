...
func newLogger() Logger {
	...
	newLog := logger{entry: logrus.NewEntry(l)}

	// Handle error by logging it and allow application to continue with default logger configuration
	if err != nil {
		newLog.Errorf(context.Background(), "Error parsing logger config: %s", err)
	}
	return newLog
}
